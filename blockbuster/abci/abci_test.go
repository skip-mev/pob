...
func (s *ABCITestSuite) TestPrepareProposal() {
	...
	s.Run("can build a proposal where first lane cannot fit txs but second lane can fit its second tx", func() {
		// Create a bid tx that includes a single bundled tx
		tx1, err := testutils.CreateRandomTx(
			s.encodingConfig.TxConfig,
			s.accounts[0],
		...
		// Set up the default lane with the two transactions
		defaultLane := s.setUpDefaultLane(math.LegacyMustNewDecFromStr("0.0"), map[sdk.Tx]bool{
			tx1: true,
			tx2: true,
		})
		...
		proposalHandler := s.setUpProposalHandlers([]blockbuster.Lane{tobLane, defaultLane}).PrepareProposalHandler()
		proposal := s.getTxBytes(tx1, tx2)

		// In this case the second transaction should be too large and should not be selected even though
		// it has a higher priority than the first transaction
		size := int64(len(proposal[1]) - 1)
		resp, err := proposalHandler(s.ctx, &cometabci.RequestPrepareProposal{MaxTxBytes: size})
		s.Require().NoError(err)
		s.Require().NotNil(resp)
...
func (s *ABCITestSuite) getTxBytes(txs ...sdk.Tx) [][]byte {
