...
// CompareFn is a function that compares 'a' to 'b'.
...
func New[T any](less CompareFn[T]) *Heap[T] {
	return &Heap[T]{
		data: make([]T, 0),
		less: less,
	}
...
func down[T any](h []T, i int, less CompareFn[T]) {
	for {
		left, right := 2*i+1, 2*i+2
		if left >= len(h) || left < 0 {
...
func up[T any](h []T, i int, less CompareFn[T]) {
	for {
		parent := (i - 1) / 2
