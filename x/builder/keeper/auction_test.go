...
func (suite *KeeperTestSuite) TestValidateBidInfo() {
	...
	cases := []struct {
		...
		},
		{
			...
			func() {
				bid = sdk.NewCoin("foo", sdk.NewInt(1000))
				balance = sdk.NewCoin("foo", sdk.ZeroInt())
			},
			false,
		},
		{
		...
		{
			"minimum bid amount and bid amount are different",
			func() {
				bid = sdk.NewCoin("foo", sdk.NewInt(1001))
				minBidIncrement = sdk.NewCoin("foo1", sdk.NewInt(1000))
			},
			false,
		},
	}
