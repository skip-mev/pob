	...
	testCases := []struct {
	...
	}{
		{
			...
			test: func() {
				...
				escrowBalance := s.queryBalanceOf(escrowAddress.String(), app.BondDenom)
				...
				s.Require().Equal(escrowBalance.Add(expectedEscrowFee), s.queryBalanceOf(escrowAddress.String(), app.BondDenom))
			},
		},
		{
	...
	testCases := []struct {
		...
		},
		{
			...
			test: func() {
				...
				// Ensure that the escrow account has the correct balance
				expectedEscrowFee := s.calculateProposerEscrowSplit(bid)
				s.Require().Equal(expectedEscrowFee.Add(escrowBalance), s.queryBalanceOf(escrowAddress.String(), app.BondDenom))
...
func (s *IntegrationTestSuite) TestInvalidBids() {
	...
	for _, tc := range testCases {
		...
		s.Require().Equal(escrowBalance, s.queryBalanceOf(escrowAddress.String(), app.BondDenom))
	}
}
